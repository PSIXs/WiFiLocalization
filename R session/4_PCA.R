...
# load dataset
data(iris)
data <- lg_40_routers_8_classes.normalized[,-41] 
...
# visualize PCA model
dataset = data.frame(species = lg_40_routers_8_classes.normalized[,"signal"], pca = PCAModel$x)
plt <- ggplot(dataset) + 
  geom_point(aes(pca.PC1, pca.PC2, colour = species, shape = species), size = 2.0) + 
